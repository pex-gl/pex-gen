...
function Loft(path, options) {
  ...
  if (options.closed) options.caps = false;
...
Loft.prototype.buildGeometry = function(caps) {
  ...
  for (var i=0; i<this.frames.length; i++) {
    for (var j=0; j<numSegments; j++) {
      if (i < numSteps - 1) {
        this.faces.push([index + (j + 1) % numSegments, index + j, index + j + numSegments ]);
        this.faces.push([index + (j + 1) % numSegments + numSegments, index + (j + 1) % numSegments, index + j + numSegments]);
      }
...
Loft.prototype.sampleTangents = function(path, numSteps) {
  var points = [];
  for(var i=0; i<numSteps; i++) {
    points.push(path.getTangentAt(i / (numSteps - 1)));
  }
  return points;
};
